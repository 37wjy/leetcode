...
//dp:DP数组，dp[l][r][j]表示吃完[l.r]范围内的青草时的最小答案。j只有0，1两种取值，分别表示奶牛吃完最后一颗草后停在青草l或者r上
int dp[N+2][N+2][2];
...
int getAnswer(int n, int k, vector<int> x) {
    ...
    for(int i=1; i<=n; ++i)
        dp[i][i][0]=dp[i][i][1]=abs(x[i]-k)*n; //设置边界条件只吃一棵草的情况是什么
    for(int len=1; len<n; ++len)
        for(int l=1,r; (r=l+len)<=n; ++l){//枚举区间注意一个细节：先枚举区间长度，再枚举左端点，并自然求出右端点
            //进行转移
